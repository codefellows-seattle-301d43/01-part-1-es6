...
function Product(name) {
  this.name = name;
  this.path = `img/${name}.jpg`; //changed to template literal notation
  this.votes = 0;
  this.views = 0;
...
function displayPics(){
  ...
  // The error "rando is not defined" occurred because rando was declared within the block of code, or function displayPics. This makes rando local in scope and global, therefore it is not defined outside the block.
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`${event.target.id} has ${allProducts[i].votes} votes in ${allProducts[i].views} views`); //changed to template literal notation
    }
  }
...
function showList() {
  for(let i = 0; i < allProducts.length; i++) {
    ...
    liEl.textContent = `${allProducts[i].name} has ${allProducts[i].votes} votes in ${allProducts[i].views} views`; //changed to template literal notation
